...
function ParentLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    const settings = loadSettings();
    const child = studentsData.find(
      (s) => s.name.toLowerCase() === parentChildName.toLowerCase() && 
      (
         s.guardianName?.toLowerCase() === parentName.toLowerCase() ||
         s.fatherName?.toLowerCase() === parentName.toLowerCase() ||
         s.motherName?.toLowerCase() === parentName.toLowerCase()
      ) &&
      (
    ...
    if (child) {
      if (settings.blockedUsers.includes(parentName)) {
        setLoginError("Your parent account has been blocked by the administrator.");
        logFailedLogin(parentName, "Attempted parent login while blocked");
        return;
      }
      localStorage.setItem("session_user", JSON.stringify({ role: "parent", id: child.id, name: parentName, childName: child.name }));
      logLogin(parentName, parentPhone, "parent", { loginMethod: "phone" });
      navigate({ to: "/parent" });
    } else {
      setLoginError("Invalid parent credentials. Details must match school records.");
      logFailedLogin(parentName || "unknown", "Invalid parent credentials");
    }
  };
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-12">
        ...
        >
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground">Your Name (Parent/Guardian)</label>
              <input
                type="text"
                value={parentName}
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-foreground">Telephone Number</label>
              <input
                type="tel"
                value={parentPhone}
