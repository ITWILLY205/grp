...
function TeacherAssignment() {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      // Flatten teacher assignments into table rows
      const flat: FlatAssignment[] = [];
      for (const teacher of teachers) {
        const teacherName = teacher.user?.full_name || "Unknown";
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-gray-900">Current Teacher Assignments</h2>
        ...
        <table className="w-full">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr>
                <td colSpan={6} className="px-4 py-8 text-center text-gray-500">
                  No assignments found. Click "New Assignment" to assign teachers to subjects and classes.
                </td>
              </tr>
            ) : (
              assignments.map((assignment) => (
                <tr key={assignment.id} className="border-t border-gray-200 hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{assignment.teacher}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{assignment.subject}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{assignment.className}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{assignment.stream}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{assignment.term}</td>
                  <td className="px-4 py-3 text-sm text-gray-600">{assignment.year}</td>
                </tr>
              ))
            )}
          </tbody>
